...
export const EmployeeStatusBadge: React.FC<EmployeeStatusBadgeProps> = ({ status, className }) => {
  // Normalize key to handle potentially inconsistent API response formats safely
  const normalizedStatus = (status?.toUpperCase() || 'ACTIVE') as EmployeeStatus;

  const currentStyle = statusStyles[normalizedStatus] ?? statusStyles.ACTIVE;
  const currentIcon = statusIcons[normalizedStatus] ?? statusIcons.ACTIVE;
  const currentLabel = statusLabels[normalizedStatus] ?? statusLabels.ACTIVE;

  return (
    <span
      className={cn(
        'inline-flex items-center gap-1 px-2.5 py-1 rounded-full font-label-sm text-label-sm shadow-sm select-none shrink-0',
        currentStyle,
        className
      )}
    >
