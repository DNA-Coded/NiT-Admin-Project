...
export const EmployeeDrawer: React.FC<EmployeeDrawerProps> = ({ 
  ...
  return (
    <>
      ...
      {/* Side Drawer Panel */}
      <div 
        className="fixed right-0 top-0 h-full w-full max-w-md bg-surface-container-lowest border-l border-outline-variant shadow-lg z-50 flex flex-col transition-transform duration-300 transform translate-x-0 overflow-hidden"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-outline-variant flex items-center justify-between bg-surface-container-lowest shrink-0">
          <h3 id="drawer-title" className="font-headline-md text-headline-md text-on-background">
            Employee Details
          </h3>
          <button
        ...
        {/* Dynamic Inner Scrolling Context Block */}
        <div className="flex-1 p-6 flex flex-col gap-6 overflow-y-auto no-scrollbar">
          
          {/* Profile Overview */}
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-primary-fixed text-on-primary-fixed flex items-center justify-center font-headline-lg font-bold text-2xl shrink-0 relative">
              {employee.avatarUrl ? (
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h4 className="font-headline-md text-headline-md text-on-background truncate">{employee.name}</h4>
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold shrink-0 ${employee.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                  {employee.isActive ? 'ACTIVE' : 'INACTIVE'}
                </span>
              </div>
              <p className="font-body-sm text-body-sm text-on-surface-variant truncate">{employee.employeeId}</p>
          ...
          <div className="bg-surface-container-low border border-outline-variant p-4 rounded-lg flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3 font-body-sm text-body-sm text-on-surface">
              <div className="flex items-center gap-2 min-w-0">
                <span className="material-symbols-outlined text-[18px] text-outline shrink-0">mail</span>
                <span className="truncate">{employee.email || 'N/A'}</span>
              </div>
              <div className="flex items-center gap-2 min-w-0">
                <span className="material-symbols-outlined text-[18px] text-outline shrink-0">call</span>
            ...
            {employee.biometricDevice ? (
              <div className="flex flex-col gap-1.5 font-body-sm text-body-sm text-on-surface">
                <div className="flex items-center gap-2 text-success font-medium">
                  <span className="material-symbols-outlined text-[18px] shrink-0">fingerprint</span>
              ...
              <div className="flex items-center gap-2 text-on-surface-variant font-body-sm text-body-sm">
                <span className="material-symbols-outlined text-[18px] text-outline shrink-0">cancel</span>
                <span>No Biometric Mapping</span>
              </div>
            )}
          </div>

          {/* Attendance Summary Data Cards */}
          <div className="bg-surface-container-low border border-outline-variant p-4 rounded-lg flex flex-col gap-3">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              <div className="bg-success-bg/30 border border-success/20 p-2 rounded">
                ...
                <span className="text-[10px] text-on-surface-variant font-medium block truncate">Present</span>
              </div>
              <div className="bg-danger-bg/30 border border-danger/20 p-2 rounded">
                ...
                <span className="text-[10px] text-on-surface-variant font-medium block truncate">Absent</span>
              </div>
              <div className="bg-warning-bg/30 border border-warning/20 p-2 rounded">
                ...
                <span className="text-[10px] text-on-surface-variant font-medium block truncate">Late</span>
              </div>
              <div className="bg-surface-container-high border border-outline-variant/30 p-2 rounded">
                ...
                <span className="text-[10px] text-on-surface-variant font-medium block truncate">Leaves</span>
              </div>
            </div>
          </div>
        </div>

        {/* Static Actions Control Footer Strip */}
        <div className="p-6 border-t border-outline-variant bg-surface-container-low shrink-0 flex flex-col gap-3">
