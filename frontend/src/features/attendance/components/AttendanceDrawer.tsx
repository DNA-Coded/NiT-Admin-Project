...
import type { CorrectAttendancePayload } from '../types/attendance.api.types';
...
interface AttendanceDrawerProps {
  ...
  onClose: () => void;
  onCorrect?: (id: string, payload: CorrectAttendancePayload) => Promise<void>;
}
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-surface-container-lowest border-l border-outline-variant shadow-lg z-50 flex flex-col transition-transform duration-300 transform translate-x-0 overflow-y-auto no-scrollbar">
        ...
        <div className="px-6 py-5 border-b border-outline-variant flex items-center justify-between bg-surface-container-low">
          <h3 className="font-headline-md text-headline-md text-on-background">Attendance Record Details</h3>
          ...
          {isEditing ? (
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 bg-surface-container p-4 rounded-lg border border-outline-variant">
              ...
              <div className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) => setEditForm(p => ({ ...p, status: e.target.value as AttendanceStatus }))}
            ...
            <>
              ...
              <div className="bg-surface-container-low border border-outline-variant p-4 rounded-lg flex flex-col gap-3">
                <h5 className="font-label-md text-label-md text-primary font-bold">Punch Summary</h5>
                <div className="grid grid-cols-3 gap-2 font-body-sm text-body-sm text-on-surface border-b border-outline-variant/60 pb-3">
                  <div>
                    <span className="block text-on-surface-variant font-label-sm text-label-sm">First In</span>
                    <span className="font-bold font-mono text-success">{record.firstIn || '--'}</span>
                  </div>
                  <div>
                    <span className="block text-on-surface-variant font-label-sm text-label-sm">Last Out</span>
                    <span className="font-bold font-mono text-primary">{record.lastOut || '--'}</span>
                  </div>
                  <div>
                    <span className="block text-on-surface-variant font-label-sm text-label-sm">Total Hours</span>
                    <span className="font-bold font-mono">{record.totalHours || '--'}</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2 text-xs text-on-surface-variant pt-1">
                  <div>
                    <span className="block font-medium text-on-surface">Department:</span>
                    <span>{record.department || 'General'}</span>
                  </div>
                  <div>
                    <span className="block font-medium text-on-surface">Designation:</span>
                ...
                <div className="flex flex-col gap-1 font-body-sm text-body-sm text-on-surface">
                  {record.deviceUsed ? (
                    ...
                    <div className="flex items-center gap-2 text-on-surface-variant italic">
                      ...
                      <span>No specific device log attached.</span>
                    </div>
                  )}
                </div>
              </div>
