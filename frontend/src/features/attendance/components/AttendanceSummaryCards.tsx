...
export const AttendanceSummaryCards: React.FC<AttendanceSummaryCardsProps> = ({ summary }) => {
  ...
  const lateArrivals = summary?.lateArrivals ?? 0;
  const earlyDepartures = summary?.earlyDepartures ?? 0;
  const avgWorkingHours = summary?.avgWorkingHours || '--';
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
      ...
      <div className="bg-surface-container-lowest border border-outline-variant p-4 rounded flex flex-col justify-between h-24 border-l-4 border-l-primary shadow-sm">
        ...
        <div className="font-display-lg text-2xl font-bold text-on-background">
          {earlyDepartures.toLocaleString()}
        </div>
      </div>
