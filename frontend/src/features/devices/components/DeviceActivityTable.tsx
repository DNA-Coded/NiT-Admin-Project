...
interface DeviceActivityTableProps {
  ...
  onRefresh?: () => void; // <-- Added explicit refresh callback
  ...
  return (
    <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm flex flex-col h-full">
      <div className="px-6 py-4 border-b border-outline-variant flex items-center justify-between bg-surface-container-low">
        ...
        <button
          className="text-label-sm text-outline font-medium hover:text-primary transition-colors flex items-center gap-1 disabled:opacity-50"
          onClick={onRefresh}
          disabled={viewState === 'loading'}
        >
          <span className={`material-symbols-outlined text-[16px] ${viewState === 'loading' ? 'animate-spin' : ''}`}>
            refresh
          </span>
