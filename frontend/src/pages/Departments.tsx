...
export default function Departments() {
  ...
  // Real count states fetched from primary sources
  const [totalSystemEmployees, setTotalSystemEmployees] = useState<number | undefined>(undefined);
  const [totalSystemDevices, setTotalSystemDevices] = useState<number | undefined>(undefined);
  ...
  // Fetch real metrics for Employees and Active Devices in parallel
  useEffect(() => {
    const fetchSystemMetrics = async () => {
      // 1. Fetch total employee count
      try {
        const empResponse = await facultyService.getAllFaculty({ limit: 1 });
        const totalEmp = empResponse.pagination?.totalCount || 0;
        setTotalSystemEmployees(totalEmp);
      } catch (err) {
        console.error("Failed to fetch total system employee count", err);
      }

      // 2. Fetch total ACTIVE device count
      try {
        // Querying with isActive: true to filter for active devices only
        const deviceResponse = await deviceService.getDevices({ limit: 1, isActive: true });
        const res = deviceResponse as any;

        // Extracts total count from API pagination metadata
        const totalDev = res?.pagination?.total ?? res?.pagination?.totalCount ?? 0;

        setTotalSystemDevices(totalDev);
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <DepartmentSummaryCards 
        ...
        meta={meta} 
        totalEmployees={totalSystemEmployees}
        totalDevices={totalSystemDevices}
      />
